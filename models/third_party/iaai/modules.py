"""Image-as-an-IMU geometry components; see the adjacent MIT LICENSE.

MSCAN and the Light Hamburger Decoder are adapted from SegNeXt:
...
class ConvModule(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, padding: int=0, use_norm: bool=False, bias: bool=True):
...
class NMF2D(nn.Module):
    ...
    def compute_coef(self, x: torch.Tensor, bases: torch.Tensor, coef: torch.Tensor) -> torch.Tensor:
        ...
        numerator = torch.bmm(x.transpose(1, 2), bases)
        denominator = coef.bmm(bases.transpose(1, 2).bmm(bases))
        return coef * numerator / (denominator + 1e-06)
...
class StemConv(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int):
        ...
        """
        super().__init__()
        self.proj = nn.Sequential(nn.Conv2d(in_channels, out_channels // 2, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)), nn.BatchNorm2d(out_channels // 2), nn.GELU(), nn.Conv2d(out_channels // 2, out_channels, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)), nn.BatchNorm2d(out_channels))
    ...
    def __init__(self):
        ...
        for i in range(self.num_stages):
            if i == 0:
                ...
                patch_embed = OverlapPatchEmbed(patch_size=7 if i == 0 else 3, stride=4 if i == 0 else 2, in_chans=self.in_chans if i == 0 else self.embed_dims[i - 1], embed_dim=self.embed_dims[i])
            block = nn.ModuleList([Block(dim=self.embed_dims[i], mlp_ratio=self.mlp_ratios[i], drop=self.drop_rate) for _ in range(self.depths[i])])
